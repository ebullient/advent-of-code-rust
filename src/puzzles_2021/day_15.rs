...
pub fn run() {
    ...
    let big_path = astar(&g, (0,0), |finish| finish == big_end, |e| *e.weight(), |_| 0).unwrap();
    println!("** Part 2 Final: {:?}", big_path.0);
}

fn parse(input: &Vec<String>) -> (DiGraphMap<(usize,usize), i32>, (usize, usize), (usize, usize)) {
    ...
    (graph, (height - 1, width - 1), (max_height - 1, max_width - 1))
}

fn add_edges(graph: &mut DiGraphMap<(usize,usize), i32>, max_height: usize, max_width: usize, y: usize, x: usize, risk: i32) {
    let here = (y, x);
    if y > 0 {
...
mod tests {
    ...
    #[test]
    fn test() {
        let input: Vec<String> = puzzle_input::split_string("1163751742
            1381373672
            2136511328
            3694931569
            7463417111
