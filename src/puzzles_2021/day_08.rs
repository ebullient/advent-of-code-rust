...
fn decipher(input: &Vec<String>) -> HashMap<char, char> {
    ...
    possible[5].retain(|c| input[0].contains(c) && input.iter().filter(|x| x.len() == 6).all(|x| x.contains(c)));
    ...
    possible[6].retain(|c| input.iter().filter(|x| x.len() > 4).all(|x| x.contains(c)) && !actual.contains(&c));
    actual[6] = possible[6].chars().next().unwrap();

    // input[2] has length 4 (wires 1 and 3)
    // input length == 5: all will have wire 3 (not wire 1)
    possible[3].retain(|c| input[2].contains(c) && input.iter().filter(|x| x.len() ==5).all(|x| x.contains(c)) && !actual.contains(&c));
...
mod tests {
    ...
    #[test]
    fn test() {
        let single_test: Vec<String> = vec![String::from("acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf")];
        ...
        let input: Vec<String> = puzzle_input::split_string("be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
